...
func (l *LinodeClient) GetIP(cluster *clusterv1.Cluster, machine *clusterv1.Machine) (string, error) {
	instance, err := l.instanceExists(machine)
	if err != nil {
		return "", err
	}
	if instance == nil {
		return "", fmt.Errorf("instance %s doesn't exist", instance.Name)
	}
	if len(instance.Networks.V4) == 0 {
		return "", fmt.Errorf("instance %s doesn't have IP address assigned", instance.Name)
	}
	return instance.Networks.V4[0].IPAddress, nil
}

// GetKubeConfig returns kubeconfig from the master.
func (l *LinodeClient) GetKubeConfig(cluster *clusterv1.Cluster, master *clusterv1.Machine) (string, error) {
	instance, err := l.instanceExists(master)
	if err != nil {
		return "", err
	}
	if instance == nil {
