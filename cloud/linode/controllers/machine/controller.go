...
import (
	...
	machineactuator "github.com/displague/cluster-api-provider-linode/cloud/linode/actuators/machine"
	"github.com/displague/cluster-api-provider-linode/cloud/linode/actuators/machine/machinesetup"
	"github.com/displague/cluster-api-provider-linode/cloud/linode/controllers/machine/options"
)

const (
	controllerName = "linode-machine-controller"
)
...
func Start(server *options.Server, recorder record.EventRecorder, shutdown <-chan struct{}) {
	...
	if err != nil {
		glog.Fatalf("Could not create linode machine actuator: %v", err)
	}
