...
.PHONY: generate
generate:
	go build -o $$GOPATH/bin/deepcopy-gen github.com/displague/cluster-api-provider-linode/vendor/k8s.io/code-generator/cmd/deepcopy-gen
	deepcopy-gen \
	  -i ./cloud/linode/providerconfig,./cloud/linode/providerconfig/v1alpha1 \
...
install: ## Install cluster-controller, machine-controller and clusterctl
	CGO_ENABLED=0 go install -ldflags '-extldflags "-static"' github.com/displague/cluster-api-provider-linode/cmd/cluster-controller
	CGO_ENABLED=0 go install -ldflags '-extldflags "-static"' github.com/displague/cluster-api-provider-linode/cmd/machine-controller
	CGO_ENABLED=0 go install -ldflags '-extldflags "-static"' github.com/displague/cluster-api-provider-linode/clusterctl
