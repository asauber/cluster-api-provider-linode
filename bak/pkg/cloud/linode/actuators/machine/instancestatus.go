...
func (l *LinodeClient) instanceStatus(machine *clusterv1.Machine) (instanceStatus, error) {
	if l.v1Alpha1Client == nil {
		return nil, nil
	}
	currentMachine, err := util.GetMachineIfExists(l.v1Alpha1Client.Machines(machine.Namespace), machine.ObjectMeta.Name)
	if err != nil {
		return nil, err
	}
	...
	return l.machineInstanceStatus(machine)
}

// machineInstanceStatus converts string from annotation to the machine object.
func (l *LinodeClient) machineInstanceStatus(machine *clusterv1.Machine) (instanceStatus, error) {
	...
	serializer := json.NewSerializer(json.DefaultMetaFactory, l.scheme, l.scheme, false)
...
func (l *LinodeClient) setMachineInstanceStatus(machine *clusterv1.Machine, status instanceStatus) (instanceStatus, error) {
...
func (l *LinodeClient) updateInstanceStatus(machine *clusterv1.Machine) error {
	if l.v1Alpha1Client == nil {
		return nil
	}
	...
	m, err := l.setMachineInstanceStatus(currentMachine, status)
	if err != nil {
		return err
	}

	_, err = l.v1Alpha1Client.Machines(machine.Namespace).Update(m)
	return err
}
